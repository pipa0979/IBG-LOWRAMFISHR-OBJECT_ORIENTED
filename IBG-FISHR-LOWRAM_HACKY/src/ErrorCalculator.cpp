...
void ErrorCalculator::readBmidFile(std::string path)
{
	...
	std::string item,line;
     try
     {
       ...
       if( !file_bmid )
       {
    	   std::cerr<< "Error: bmid file cannot be oppened. This could be due to a bad path or a currupted program. Exiting program" <<std::endl;
            exit( -1 );
       }
       Marker cur_marker;
...
void ErrorCalculator::changeMapFile( std::string path )
{
     ...
     try
     {
       ...
       if( !file_bmid )
       {
    	   std::cerr<< "Error: hold out file cannot be oppened. This could be due to a bad path or a currupted program. Exiting program" <<std::endl;
            exit( -1 );
       }
...
void ErrorCalculator::readBsidFile(std::string path)
{
   try
   {
       ...
       if( !file_bsid )
       {
    	   std::cerr<< "Error: bsid file cannot be oppened. This could be due to a bad path or a currupted program. Exiting program" <<std::endl;
            exit( -1 );
       }

       std::stringstream ss;
       std::string item,line;
       while( std::getline(file_bsid , line) )
        {
                ss.clear();ss.str(line);
...
void ErrorCalculator::readPedFile(std::string path, std::string missing)
{
         if(pers_count<=0)
         {
        	 std::cerr<<"Warning: You must read bsid file before reading the ped file. Consider changing the argument order at the command line."<<std::endl;
             return;
         }
         ped_file.clear();
         ...
         try
         {
        	 std::ifstream file_ped(path.c_str());
                if( !file_ped )
                {
                	std::cerr<< "Error: original ped file cannot be oppened. This could be due to a bad path or a currupted program. Exiting program" <<std::endl;
                     exit( -1 );
                }
...
std::vector<float>  ErrorCalculator::getMovingAverages(std::vector<int> errors,int snp1,int snp2,int width)
{
	std::vector<float> averages;
	int length= snp2-snp1;
	std::vector<int> e_Places;
...
void ErrorCalculator::finalOutPut(int pers1,int pers2,int snp1,int snp2, float min_cm)
{
        ...
        if(sample_id.size()<=pers1*2||sample_id.size()<=pers2*2) 
        {
	     ...
	     //trying to change format for sorting. Removeing first 5 chars to erase "pers." PIYUSH CHNAGED NAMES INTACT
